...
def temporal_loss(ph_image1, ph_image2, ph_flow, width, height):
    warped_out = wf.warp_image(ph_image1, ph_flow, width, height)
    D = float(width * height * 3)
    loss = 1.0 / D * tf.reduce_sum(tf.nn.l2_loss(tf.to_float(warped_out) - tf.to_float(ph_image2)))
