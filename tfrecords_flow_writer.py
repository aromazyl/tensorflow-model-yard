...
def _calc_opticalflow(image_prev, image_next):
    return iu.GetImageFlow(image_prev, image_next)

def _convert_to_example(filename_prev, filename_next, optical_flow, image_buffer_prev, image_buffer_next, height, width):
  ...
  example = tf.train.Example(features=tf.train.Features(feature={
      ...
      'image/filename_prev': _bytes_feature(tf.compat.as_bytes(os.path.basename(filename_prev))),
      'image/filename_curr': _bytes_feature(tf.compat.as_bytes(os.path.basename(filename_curr))),
      'image/encoded_prev': _bytes_feature(tf.compat.as_bytes(image_buffer_prev))})),
      'image/encoded_curr': _bytes_feature(tf.compat.as_bytes(image_buffer_next))})),
      'image/optical_flow': _bytes_feature(tf.compat.as_bytes(optical_flow))
...
def _process_image_batch(coder, filenames, )
